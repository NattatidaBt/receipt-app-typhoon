...
# =========================================================
# GLOBAL DESIGN CSS (RecAipt Pixel-Perfect Theme)
# =========================================================
st.markdown("""
<style>
/* ── 1. ซ่อนระบบดั้งเดิมรอบแอปพลิเคชัน ── */
...
/* ── 2. ตั้งค่าโทนสีพื้นหลังขาวอมชมพูละมุนทั้งแอป ── */
.stApp { 
...
/* =========================================================
   🔥 4. ปรับดีไซน์กล่องสแกนอัปโหลดไฟล์ (ตรงตามรูปที่ 2 แบบ 100%)
========================================================= */
...
/* บังคับกางตัวโครงกล่องด้านนอกให้เป็นสีขาวโค้งมน พร้อมเส้นประสีชมพู */
...
/* ทำลายและสั่งซ่อนเศษปุ่มดั้งเดิม ข้อมูลสเปคไฟล์ และปุ่มเลือกลงทะเบียนออกทั้งหมด */
...
/* บังคับขยายปุ่มเลือกระบบความกว้างภายในให้โปร่งแสงเต็มแผ่น เพื่อให้กดคลิกลากวางได้ทุกพิกัด */
...
/* 📌 ซ้อนทับหน้ากากจำลองเอกสารพาสเทลมินิมอลไว้กึ่งกลางกล่องขาวอย่างเที่ยงตรง */
...
/* ปรับสไตล์เฉพาะไอคอนอักขระแถวแรกของตัวแปรจำลอง */
...
/* =========================================================
   5. RESULT DASHBOARD VIEWPORT (แก้ไขส่วนเกินที่งอกหลุดโครง)
========================================================= */
.result-wrapper {
    background: #FFFFFF;
    border-radius: 32px;
...
/* บังคับสยบและล้างช่องว่างของ Iframe Component ข้อมูลฝั่งขวา */
...
def build_detail_card_html(extracted_json):
    """สร้างโครงสร้างกระดาษฟอร์มรายละเอียดใบเสร็จสีชมพูพาสเทลตามแนวทางแบรนด์เล่มหน้า 42"""
...
if "processed_img" not in st.session_state or st.session_state.get("file_uploaded") is None:
    ...
    if uploaded_file is not None:
        ...
        with st.spinner("⏳ Processing image..."):
            ...
            st.session_state["processed_img"] = process_method_4_sharpening(deskewed)
...
# =========================================================
    ...
    # ── ฝั่งขวา: การ์ดฟอร์มรายละเอียดใบเสร็จสีชมพูพาสเทล ──
    with col_right:
        if has_error:
            st.error(f"❌ {extracted_json['error']}")
        else:
            items_count = len(extracted_json.get("items", []) or [])
            card_height = 440 + max(0, items_count - 2) * 38
            ...
            # บล็อกปุ่มส่งออกข้อมูลด้านล่างของการ์ดพอดีเป๊ะ
